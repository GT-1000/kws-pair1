...
const regionStrokeHover = new Stroke({ color: 'rgba(30, 136, 229, 1)', width: 3 });
...
export function shelterStyle(feature: FeatureLike) {
  ...
  const radiusBase = cap == null ? 6 : Math.max(6, Math.min(14, 4 + Math.sqrt(cap) / 3));
  ...
  return new Style({
    image: new CircleStyle({
      ...
      stroke: new Stroke({ color: 'rgba(255,255,255,0.95)', width: hovered ? 3 : 2 }),
    }),
    text: hovered
      ? new Text({
          text: cap != null ? `${cap}` : '',
